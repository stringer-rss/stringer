...
# you shouldn't do anyway. This will reduce the size of the load path, making
# `require` faster if you don't use bootsnap, or reduce the size of the
# bootsnap cache if you use it.
...
# Important: If you have data encrypted with previous Rails versions, there are
# two scenarios to consider:
#
# 1. If you have +config.active_support.key_generator_hash_digest_class+
# configured as SHA1 (the default before Rails 7.0), you need to configure
...
#
# If you don't currently have data encrypted with Active Record encryption, you
# can disable this setting to configure the default behavior starting 7.1+:
# Rails.application.config.active_record
#   .encryption.support_sha1_for_non_deterministic_encryption = false
...
# string literals.
#
# SQLite has some quirks around double-quoted string literals. It first tries
# to consider double-quoted strings as identifier names, but if they don't
# exist it then considers them as string literals. Because of this, typos can
# silently go unnoticed. For example, it is possible to create an index for a
# non existing column. See
...
# `ActiveSupport::MessagePack`. `ActiveSupport::MessagePack` can roundtrip some
# Ruby types that are not supported by JSON, and may provide improved
# performance, but it requires the `msgpack` gem.
#
# For more information, see
# https://guides.rubyonrails.org/v7.1/configuring.html#config-active-support-message-serializer
#
# If you are performing a rolling deploy of a Rails 7.1 upgrade, wherein
# servers that have not yet been upgraded must be able to read messages from
# upgraded servers, first deploy without changing the serializer, then set the
# serializer in a subsequent deploy.
# Rails.application.config
...
# defined in a model. This matches the behaviour of all other callbacks. In
# previous versions of Rails, they ran in the inverse order.
# Rails.application.config
...
# Configure Action Text to use an HTML5 standards-compliant sanitizer when it
